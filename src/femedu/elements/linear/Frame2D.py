...
class Frame2D(Element):
    ...
    * Linear elastic material (may change future releases)
    ...
    def __init__(self, nodei, nodej, material, label=None, use_p_delta=False):
        """
        ...
        :param use_p_delta: set to `True` if :math:`P-\Delta` effects shall be included
        """
        super().__init__((nodei, nodej), material, label=label)
        self.element_type = DrawElement.CURVE

        # store the P-delta flag
        self.use_p_delta = use_p_delta
